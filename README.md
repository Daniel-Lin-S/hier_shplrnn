...
## EpilepticSeizures Workflow

Facts from aeon (`EpilepticSeizures`):

1. Shape is `(11500, 1, 178)` for `split=None` (`all`).
2. This means one channel (`obs_size = 1`) and fixed length `178`.
3. Class distribution is imbalanced on `all` (`0: 2300`, `1: 9200`).
4. `train` and `test` splits have different case counts, which is not directly aligned with this codebase's
     subject-specific parameterization.

Because this repository models each case as a subject, the most stable workflow is to train/evaluate on `split=all`
and let the model's internal temporal split handle train-vs-test timesteps.

### 1 Convert aeon data to `.pt`

```bash
./.conda/bin/python aeon_to_pt.py \
...
Optional quick smoke dataset:

```bash
./.conda/bin/python aeon_to_pt.py \
    --dataset_name EpilepticSeizures \
    --split train \
    --output_path ./data/epileptic/seizures_train.pt \
    --save_auxiliary
```

### 2 Train with an EpilepticSeizures config

Use `configs/epileptic_seizures.yaml` (provided in this repository), then run:

```bash
python main.py \
    --config ./configs/epileptic_seizures.yaml \
    --data_path ./data/epileptic/seizures_all.pt \
    --eval_data_path ./data/epileptic/seizures_all.pt \
    --save_path ./trained_models \
    --experiment epileptic \
    --name projection \
    --run 1
```
...
./.conda/bin/python main_eval.py \
    --config ./configs/epileptic_seizures.yaml \
    --model_path ./trained_models/epileptic/projection \
    --save_path ./results/epileptic
```
